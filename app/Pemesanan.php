...
//PERAMALAN
public function getDetailPemesananCountAttribute($value)
{
    $pcsCount = 0;
    foreach ($this->Pemesanan as $data) {
        $pcsCount += $data->kuantitas;
    }

    return count($this->Pemesanan).' item, '.$pcsCount.' pcs';
}
...
public static function getDataPemesanan($request)
{
    $from = $request->from;
    $to = $request->to;

    $data = Pemesanan::selectRaw('DATE_FORMAT(created_at, "%Y-%m") as periode, sum(kuantitas) as total')
...
public static function getAllDataPemesanan()
{
    $array = [];
    for($i = 0; $i < 12; $i++) {
        $array[$i] = '';
    ...
    $data = Pemesanan::selectRaw('DATE_FORMAT(tanggal, "%Y-%m") as periode, sum(total) as total')
